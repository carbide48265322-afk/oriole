...
- Name: audit-policy-dimension.spec.ts >> 审核策略与审核维度管理 >> E2E-3: 审核策略新增与删除
- Location: e2e/audit-policy-dimension.spec.ts:59:7
                  ...
                  - generic [ref=e177]:
                    ...
                    - button "delete 删除" [ref=e193] [cursor=pointer]:
                      - img "delete" [ref=e195]:
                        - img [ref=e196]
                      - generic [ref=e198]: 删除
              - row "E2E 测试策略 未关联 2026/4/12 02:05:37 edit 编辑 link 关联维度 delete 删除" [ref=e199]:
                - cell "E2E 测试策略" [ref=e200]
                - cell [ref=e201]
                - cell "未关联" [ref=e202]:
                  - generic [ref=e204]: 未关联
                - cell "2026/4/12 02:05:37" [ref=e205]
                - cell "edit 编辑 link 关联维度 delete 删除" [ref=e206]:
                  - generic [ref=e207]:
                    - button "edit 编辑" [ref=e209] [cursor=pointer]:
                      - img "edit" [ref=e211]:
                        - img [ref=e212]
                      - generic [ref=e214]: 编辑
                    - button "link 关联维度" [ref=e216] [cursor=pointer]:
                      - img "link" [ref=e218]:
                        - img [ref=e219]
                      - generic [ref=e221]: 关联维度
                    - button "delete 删除" [active] [ref=e223] [cursor=pointer]:
                      - img "delete" [ref=e225]:
                        - img [ref=e226]
                      - generic [ref=e228]: 删除
  - generic [ref=e233] [cursor=pointer]:
    ...
    - generic [ref=e238]:
      ...
      - button "Collapse issues badge" [ref=e244]:
        - img [ref=e245]
  - alert [ref=e247]
  - tooltip "exclamation-circle 确定要删除此策略吗？ 取 消 确 定" [ref=e249]:
    - generic [ref=e251]:
      - generic [ref=e252]:
        - img "exclamation-circle" [ref=e254]:
          - img [ref=e255]
        - generic [ref=e258]: 确定要删除此策略吗？
      - generic [ref=e259]:
        - button "取 消" [ref=e260] [cursor=pointer]:
          - generic [ref=e261]: 取 消
        - button "确 定" [ref=e262] [cursor=pointer]:
          - generic [ref=e263]: 确 定
```
...
```ts
  16  |     // 1. 直接导航到审核策略页面
  17  |     await page.goto('/audit-policy');
  18  |     
  19  |     // 2. 验证页面显示"审核策略"标题
  20  |     await expect(page.getByText('审核策略管理')).toBeVisible({ timeout: 5000 });
  21  |     
  22  |     // 3. 验证侧边栏包含"审核策略"菜单项且高亮
  23  |     const sidebarMenuItem = page.getByRole('menuitem', { name: '审核策略' });
  24  |     await expect(sidebarMenuItem).toBeVisible();
  25  |     
  26  |     // 4. 验证显示策略列表表格（至少有一行 Mock 数据）
  27  |     await expect(page.getByText('内容安全策略')).toBeVisible({ timeout: 5000 });
  28  |     await expect(page.getByText('图片审核策略')).toBeVisible();
  29  |     
  30  |     // 5. 验证表格存在
  31  |     const table = page.locator('table');
  32  |     await expect(table).toBeVisible();
  33  |     const rows = await table.locator('tbody tr').count();
  34  |     expect(rows).toBeGreaterThan(0);
  35  |   });
  36  | 
  37  |   test('E2E-2: 审核维度列表展示与导航', async ({ page }) => {
  38  |     // 1. 直接导航到审核维度页面
  39  |     await page.goto('/audit-dimension');
  40  |     
  41  |     // 2. 验证页面显示"审核维度"标题
  42  |     await expect(page.getByText('审核维度管理')).toBeVisible({ timeout: 5000 });
  43  |     
  44  |     // 3. 验证侧边栏包含"审核维度"菜单项且高亮
  45  |     const sidebarMenuItem = page.getByRole('menuitem', { name: '审核维度' });
  46  |     await expect(sidebarMenuItem).toBeVisible();
  47  |     
  48  |     // 4. 验证显示维度列表表格（至少有一行 Mock 数据）
  49  |     await expect(page.getByText('敏感词检测')).toBeVisible({ timeout: 5000 });
  50  |     await expect(page.getByText('暴力内容识别')).toBeVisible();
  51  |     
  52  |     // 5. 验证表格存在
  53  |     const table = page.locator('table');
  54  |     await expect(table).toBeVisible();
  55  |     const rows = await table.locator('tbody tr').count();
  56  |     expect(rows).toBeGreaterThan(0);
  57  |   });
  58  | 
  59  |   test('E2E-3: 审核策略新增与删除', async ({ page }) => {
  60  |     // 1. 导航到审核策略页面
  61  |     await page.goto('/audit-policy');
  62  |     await expect(page.getByText('审核策略管理')).toBeVisible({ timeout: 5000 });
  63  |     
  64  |     // 等待数据加载
  65  |     await expect(page.getByText('内容安全策略')).toBeVisible({ timeout: 5000 });
  66  |     
  67  |     // 记录初始策略数量
  68  |     const initialRows = await page.locator('table tbody tr').count();
  69  |     expect(initialRows).toBeGreaterThan(0);
  70  |     
  71  |     // 2. 点击"新增"按钮
  72  |     await page.getByRole('button', { name: '新增策略' }).click();
  73  |     
  74  |     // 3. 验证弹出表单 Modal
