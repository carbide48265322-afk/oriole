...
test('侧边栏导航', async ({ page }) => {
  ...
  // 2. 点击侧边栏 "AI 审核" 菜单
  await page.getByRole('menuitem', { name: 'AI 审核' }).click();
  
  // 3. 验证跳转到 /ai-review
  await expect(page).toHaveURL('/ai-review');
  
  // 4. 点击侧边栏 "审核历史" 菜单
  await page.getByRole('menuitem', { name: '审核历史' }).click();
...
test('侧边栏菜单点击后手动审核页面', async ({ page }) => {
  ...
  // 点击人工复审
  await page.getByRole('menuitem', { name: '人工复审' }).click();
...
test('侧边栏菜单点击后专注复审页面', async ({ page }) => {
  ...
  // 点击审核沉浸页
  await page.getByRole('menuitem', { name: '审核沉浸页' }).click();
...
test('侧边栏菜单点击后数据统计页面', async ({ page }) => {
  ...
  // 点击统计数据
  await page.getByRole('menuitem', { name: '统计数据' }).click();
